...
local function openPage(pidx, title, script)
        ...
        for pidx, pvalue in ipairs(servoTable) do
                ...
                if pvalue.disabled ~= true then
                                ...
                                if rfsuite.config.iconsizeParam ~= 0 then
                                        if rfsuite.app.gfx_buttons["servos"][pidx] == nil then rfsuite.app.gfx_buttons["servos"][pidx] = lcd.loadMask(rfsuite.config.suiteDir .. "app/gfx/servos/" .. pvalue.image) end
                                else
